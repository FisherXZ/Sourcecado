...
fn venv_python() -> PathBuf {
    desktop_root().join(".venv/bin/python")
}
...
pub fn run() {
    ...
    let inject = format!(
        "window.__CLUB_HTTP__={http:?};window.__CLUB_API_TOKEN__={api_token:?};"
    );
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let mut server_cmd = Command::new(&python);
            server_cmd
            ...
            let child = match server_cmd.spawn() {
                Ok(child) => Some(child),
                Err(e) => {
                    eprintln!("[club] failed to start sidecar via {}: {e}", python.display());
                    None
                }
            };
            ...
            let open_i = MenuItem::with_id(app, "open", "Open Club", true, None::<&str>)?;
        ...
        .expect("error while building Club")
        .run(|app, event| {

#!/usr/bin/env bash
# Freezes the Python sidecar into a self-contained native directory and drops
# it where tauri.conf.json's `bundle.resources` expects one:
#   desktop/surfaces/gui/src-tauri/resources/sourcecado-sidecar/
#
...
# bootloader's PID, so killing that PID orphans the real sidecar. onedir has
...
#
# The frozen sidecar embeds the interpreter and all locked dependencies, so
...
BUILD_VENV="$(mktemp -d)/sidecar-build-venv"
WORKPATH="$(mktemp -d)"
trap 'rm -rf "$BUILD_VENV" "$WORKPATH"' EXIT

echo "==> building sourcecado-sidecar"
...
"$BUILD_VENV/bin/pyinstaller" \
  ...
  "$DESKTOP_DIR/packaging/sidecar_entry.py"

mkdir -p "$RESOURCE_DIR"
rm -rf "$RESOURCE_DIR/sourcecado-sidecar"
cp -R "$WORKPATH/dist/sourcecado-sidecar" "$RESOURCE_DIR/sourcecado-sidecar"
chmod +x "$RESOURCE_DIR/sourcecado-sidecar/sourcecado-sidecar"
echo "==> wrote $RESOURCE_DIR/sourcecado-sidecar/"

...
- Raw shell arguments remain in an in-memory approval vault only while the sidecar is alive. Conversation, event, and Inbox persistence contain redacted arguments; an approval resumed without its transient inputs fails closed and must be requested again.

...
- Reads and reversible writes run automatically inside their grant. Existing-file writes require the observed SHA-256 hash. Writes use a same-directory temporary file, flush and `fsync`, then atomic replacement under a per-path lock.
...
- Direct-host allow-always authority is bound to the exact command, resolved working directory, sanitized environment fingerprint, execution target, executable hash, and referenced script hashes. Only the digest and safe inspection metadata persist. The grant is visible and revocable in Settings and can satisfy an AFK call only while every component still matches.
- Shell tasks have durable IDs and bounded incremental output. A restart changes an unresolved running task to `interrupted`; Sourcecado never infers success.
- Workspace receipts are append-only and contain decisions, targets, hashes, actor/run identity, timing, exit state, truncation, and a sanitized summary. They never contain file bodies, raw environment values, terminal secrets, or unbounded output.

...
- **Capabilities trimmed to `core:default`.** The frontend has no `@tauri-apps/api` or plugin-dialog call sites today (confirmed by search), and the window-hide/show/focus calls in `lib.rs` are direct Rust API calls, not JS-invoked commands, so they need no capability grant either.

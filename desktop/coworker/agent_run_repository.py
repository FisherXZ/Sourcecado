...
@dataclass(frozen=True)
class AdoptedExternalCompletionLease:
    lease: AgentRunLease
    run: dict[str, Any]
...
class _AgentRunRepositoryBase:
    ...
    def resume_and_adopt_external_completion(
        self,
        run_id: str,
        owner_id: str,
        expected_version: int,
        ...
        lease_seconds: int | float,
        now: datetime | None = None,
    ) -> AdoptedExternalCompletionLease | None:
        """Acquire and adopt one external completion in a single transaction."""
        owner = _lease_owner(owner_id)
        interaction = _bounded_id(interaction_id, "interaction_id")
        seconds = _lease_seconds(lease_seconds)
        stamp = _timestamp(now)
        ...
        with self._lock:
            ...
            try:
                ...
                existing_receipt = next(
                    (
                        receipt
                        for receipt in continuation.get(
                            "completed_tool_receipts", []
                        )
                        if receipt.get("call_id") == interaction
                ...
                if (
                    ...
                    and inbox_row is not None
                ):
                    checkpoint_row = self._conn.execute(
                        """
                        SELECT * FROM agent_run_checkpoints
                        WHERE run_id = ? AND kind = 'tool_completed'
                        ORDER BY sequence DESC LIMIT 1
                        """,
                        (run_id,),
                    ).fetchone()
                    self._conn.commit()
                    return AdoptedExternalCompletionLease(
                        lease=_lease_from_row(row),
                        run=_run_row(row),
                        checkpoint=_checkpoint_row(checkpoint_row),
                        inbox=project_inbox_row(inbox_row),
                    )
                if (
                ...
                try:
                    result = json.loads(
                        str(inbox_row["execution_result"] or "")
                    )
                ...
                adopted = adopt_waiting_external_completion_transition(
                    continuation, run_id, ok, digest
                )
                ...
                cursor = self._conn.execute(
                    ...
                    UPDATE agent_runs SET
                        current_state = 'running', lease_owner = ?,
                        lease_expires_at = ?, checkpoint_sequence = ?,
                        source_refs = ?, artifact_refs = ?, usage = ?,
                        continuation = ?,
                        version = version + 1, updated_at = ?
                    WHERE run_id = ? AND version = ?
...
class AgentRunRepository(_AgentRunRepositoryBase):
        ...
        with self._lock:
            ...
            try:
                ...
                if (
                    ...
                    not in {"pending", "executing", "succeeded", "failed"}
                ):
                    raise ValueError(
    ...
    def validate_transcript_prefix(
        ...
        return self.validate_projection_prefix(
            run_id, messages, projection="transcript"
        )

    def validate_event_prefix(
        self, run_id: str, events: list[dict[str, Any]]
    ) -> Literal["exact", "extra_tail", "mismatch"]:
        return self.validate_projection_prefix(run_id, events, projection="event")

    def validate_projection_prefix(
        self,
        run_id: str,
        values: list[dict[str, Any]],
        *,
        projection: Literal["transcript", "event"],
    ) -> Literal["exact", "extra_tail", "mismatch"]:
        with self._lock:
            row = self._conn.execute(
        ...
        if not isinstance(cursor, dict):
            return "mismatch"
        count = cursor.get(f"{projection}_prefix_count")
        digest = cursor.get(f"{projection}_prefix_sha256")
        if (
    ...
    def mark_projection_mismatch(
        ...
        """Fail one unleased interrupted continuation closed before resume."""
        safe_projections = sorted(
            {value for value in projections if value in {"transcript", "event"}}
        )
        return self._mark_interrupted_review_required(
            run_id,
            expected_version,
            kind="projection_mismatch",
            reason="projection_mismatch",
            details={"projections": safe_projections},
            now=now,
        )

    def mark_resume_review_required(
        self,
        run_id: str,
        expected_version: int,
        reason: str,
    ...
    def repair_completed_tool_receipt(
        self,
        run_id: str,
        expected_version: int,
        now: datetime | None = None,
    ) -> dict[str, Any] | None:
        """Advance a stale pending cursor when its receipt already committed."""
        stamp = _timestamp(now)
        with self._lock:
            self._conn.execute("BEGIN IMMEDIATE")
            try:
                ...
                cursor_data = dict(continuation.get("cursor", {}))
                pending = continuation.get("pending_tool")
                ...
                next_index = int(cursor_data.get("next_tool_index", 0))
                expected_count = int(cursor_data.get("expected_tool_count", 0))
                if (
                    str(row["current_state"]) != "interrupted"
                    or row["lease_owner"] is not None
                    or row["lease_expires_at"] is not None
                    or int(row["version"]) != int(expected_version)
                    or cursor_data.get("phase") != "tools_ready"
                    or not isinstance(pending, dict)
                    or pending.get("status") != "retry_ready"
                    or pending.get("budget_reserved") is not True
                    or matching_receipt is None
                    or next_index >= expected_count
                ):
                    self._conn.commit()
                    return None
                cursor_data["next_tool_index"] = next_index + 1
                continuation = {
                    **continuation,
                    "cursor": cursor_data,
                }
                continuation.pop("pending_tool", None)
                continuation = project_continuation(continuation)
                sequence = int(row["checkpoint_sequence"]) + 1
    ...
    def _mark_interrupted_review_required(
        self,
        run_id: str,
        expected_version: int,
        *,
        kind: Literal["projection_mismatch", "process_interrupted"],
        reason: str,
        ...
        with self._lock:
            ...
            try:
                ...
                if (
                    str(row["current_state"]) == "interrupted"
                    and continuation.get("cursor", {}).get("phase")
                    == "review_required"
                ):
                    self._conn.commit()
                    return _run_row(row)
                ...
                cursor_data["phase"] = "review_required"
                continuation = project_continuation(
                    {**continuation, "cursor": cursor_data}
                )
                sequence = int(row["checkpoint_sequence"]) + 1
                updated = self._conn.execute(
                    """
